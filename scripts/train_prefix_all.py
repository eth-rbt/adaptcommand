"""
Global Prefix Training Script

This script trains a single prefix adapter on ALL training data from all 200 personas.
Unlike train_prefix_per_user.py which trains separate prefixes per persona,
this trains one unified prefix on the combined dataset.

This is the prefix tuning equivalent of train_unified_lora.py.
"""
...
def train_global_prefix(
    ...
    """
    Train a global prefix adapter on all training data.
    ...
    """
    print(f"\n{'='*60}")
    print(f"Training Global Prefix on All Training Data")
    print(f"{'='*60}\n")
    ...
    # Save final model
    print(f"\nSaving prefix adapter to {output_dir}...")
    trainer.save_model()
    ...
    print(f"[OK] Prefix adapter saved to: {output_dir}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Train global prefix adapter on all training data"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="configs/prefix_all.json",
        help="Path to training config file"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="models/prefix_all",
    ...
    # Train
    output_dir, test_metrics = train_global_prefix(
        config=config,
