"""
Unified LoRA Model Benchmark Script

This script evaluates the unified LoRA model on the smart home dataset.
"""
...
def run_benchmark(config_file: Path, lora_model_path: Path, max_examples: int = None, output_dir: Path = None):
    """Run the unified LoRA model benchmark."""
    print("="*60)
    print("UNIFIED LORA MODEL BENCHMARK")
    print("="*60)
    ...
    # Load LoRA adapter
    print(f"\nLoading LoRA adapter from {lora_model_path}...")
    ...
    if output_dir:
        ...
        results_file = output_dir / "unified_results.json"
...
def main():
    parser = argparse.ArgumentParser(description="Run unified LoRA model benchmark")
    parser.add_argument(
        "--config",
        type=str,
