"""
Train All Personas with Per-User Prefix Tuning

Trains separate prefix adapters for all 200 personas using their individual training data.
This is the batch training script for per-persona prefix tuning.
"""
...
def train_single_persona(
    ...
    """Train a single persona's prefix adapter."""
    cmd = [
        sys.executable,
        "scripts/train_prefix_per_user.py",
...
def aggregate_results(output_base_dir: Path, output_file: Path):
    ...
    for persona_dir in sorted(output_base_dir.iterdir()):
        if persona_dir.is_dir():
            metrics_file = persona_dir / "eval.json"
...
def main():
    parser = argparse.ArgumentParser(
        description="Train all personas with per-user prefix tuning"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Base output directory for all persona prefixes"
    )
    parser.add_argument(
        "--splits_file",
        type=str,
    ...
    summary_file = Path("results/prefix_per_user/prefix_per_user_summary.json")
