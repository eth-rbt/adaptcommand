...
def evaluate_cluster_lora(cluster_id=None, max_examples_per_persona=None):
    ...
    print("=" * 80)
    print("WEIGHTED MERGE COMPREHENSIVE EVALUATION")
    print("=" * 80)
    ...
    for cid in clusters_to_eval:
        ...
        # Load weighted merge model
        cluster_path = Path(f'models/weighted_cluster_{cid}')

        if not cluster_path.exists():
            print(f"Weighted merge model not found at {cluster_path}")
            print("Skipping...")
            continue
    ...
    if all_results:
        ...
        with open(output_dir / 'weighted_merge_comprehensive.json', 'w') as f:
            json.dump({
        ...
        print(f"\nSaved results to {output_dir / 'weighted_merge_comprehensive.json'}")
